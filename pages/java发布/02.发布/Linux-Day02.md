...
③. 开启访问权限(允许从外部远程访问到mysql而不是默认只能在本服务器访问mysql)
...
>
> ​	上述指令的含义为： 后台运行 java -jar 命令，并将日志输出到hello.log文件(hello.log是相对路径,意味着在你执行命令的这个路径生成日志文件)
>
> ​    & 符号:表示让命令在后台运行
...
```xml
 <!--配置具体的仓库的下载镜像-->
    ...
    <!--此镜像的唯一标识符,用来区分不同的mirror元素-->
      <id>alimaven</id>
      <!--对哪种仓库进行镜像,简单说就是替代那个仓库,这里指定central,代表中央仓库-->
      <mirrorOf>central</mirrorOf>
      <!--镜像名称-->
      <name>aliyun maven</name>
      <!--镜像URL-->
